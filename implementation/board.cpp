...
inline Location Location::Mirrored() const {
        uint x, y;
        ToCoords(_pos, x, y);
        return Location(y, x);
}
...
inline bool Location::ValidLocation(const std::string& location) {
        if (location == "swap" || location == "SWAP") {
                return true;
        }
...
inline void Board::Swap() {
        ASSERT(kBoardSize * kBoardSize - _moves_left == 1);
        ASSERT(_current == Player::Second());
        Location first_location = FirstMove();
        Load(Empty());
        _current = Player::Second();
        PlayLegal(Move(Player::Second(), first_location.Mirrored()));
}

inline bool Board::ShouldSwap(const Player &player) const {
        if (!IsValidMove(Move(player, Location::Swap()))) {
                return false;
        }
        else {
                return ::Hex::Swap::ShouldSwap(FirstMove());
        }
}
...
inline Location Board::FirstMove() const {
	for (uint i = 1; i <= kBoardSize; ++i) {
                for (uint j = 1; j <= kBoardSize; ++j) {
...
bool Board::IsValidMove(const Move& move) const {
        if (move.GetLocation() == Location::Swap()) {
                return kBoardSize * kBoardSize - _moves_left == 1
                        && _current == Player::Second();
        }
